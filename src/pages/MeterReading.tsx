...
const MeterReadingPage = () => {
  ...
  const setOpenFormConsumption = useAppStore(
    (state) => state.setOpenFormConsumption,
  );
  ...
  return (
    <>
      ...
      >
        <Typography.Title
          level={5}
          style={{ marginBottom: "20px", color: "#4f6f52" }}
        >
      ...
      >
        <Table<ReadingRecord>
          ...
          pagination={{
            ...
            onChange: (page) =>
              fetchReadingRecords(form.getFieldValue("status"), page - 1, 10),
          }}
        />
      </Card>
      <MeterReadingDrawer />
  ...
  if (record.reading !== null && record.reading !== undefined) {
    return (
      <Space orientation="vertical" size={0}>
        <Typography.Text type="secondary">{record.reading} m³</Typography.Text>
        <Button
          type="link"
          size="small"
          onClick={() => setOpenFormConsumption(true, record)}
        >
          Editar
        </Button>
      </Space>
