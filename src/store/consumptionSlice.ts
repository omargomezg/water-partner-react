...
export const createConsumptionSlice: ImmerStateCreator<ConsumptionSlice> = (set, get) => ({
	...
	fetchReadingRecords: async (status = 'pending', page = 0, size = 10) => {
		...
		try {
			const response = await apiClient.get<ReadingRecords>(
				`/consumption/related-water-meters?status=${status}&pageIndex=${page}&pageSize=${size}`,
			);
			if (response.status === 200) {
				set((state) => {
					state.readingRecords = response.data;
				});
			}
		} catch (error) {
			console.error(error);
