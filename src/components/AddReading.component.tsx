 ...
 */
export const AddReadingComponent: FC<AddReadingComponentProps> = ({ subscriptionId, periodId }) => {
	...
	const handleOpen = async () => {
		try {
			...
			console.log(data);
			form.setFieldValue('previousValue', data.content.length > 0 ? data.content[0].value : 0);
			setOpen(true);
		} catch (error) {
			const amessage = (error as Error).message;
			console.error(amessage);
		...
		<>
			...
			>
				<p style={{ paddingBottom: '10px' }}>
					Por favor, digite los números actuales que muestra el medidor. Recuerde que el valor
					actual debe ser igual o superior a la última lectura registrada para calcular
					correctamente el consumo del mes.
				</p>
				<Form layout="vertical" form={form} onFinish={onFinish}>
					<Flex gap="middle" style={{ width: '100%' }}>
						<Form.Item label="Lectura anterior" name="previousValue" style={{ flex: 1 }}>
							<InputNumber
								readOnly
								style={{ width: '100%' }}
