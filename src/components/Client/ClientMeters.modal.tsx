...
const ClientMetersModal: FC = () => {
	...
	const handleRemoveMeter = async (meterId: number) => {
		if (clientForMeters) {
			const response = await removeClientWaterMeter(clientForMeters.rut, meterId.toString());
			if (response.success) {
				...
				<Space>
					<Tooltip title="Administrar Subsidio">
						<Button
							...
							onClick={() => openSubsidyModal(record)}
						/>
					</Tooltip>
					<Popconfirm
						title="¿Estás seguro de desasociar este medidor?"
						onConfirm={() => handleRemoveMeter(record.id)}
					>
			...
			>
				...
				<div style={{ marginTop: 16 }}>
					{!isAssociating ? (
						...
						<AssociateMeterSection
							onCancel={() => setIsAssociating(false)}
							onSuccess={() => {
								setIsAssociating(false);
								getClients();
								closeModal();
...
const AssociateMeterSection: FC<{ onCancel: () => void; onSuccess: () => void }> = ({
	onCancel,
	onSuccess,
}) => {
	...
	const onFinishLink = async (values: any) => {
		...
		setLoading(true);
		const success = await associateWaterMeterToClient(clientForMeters.rut, values.meterId);
		setLoading(false);
		if (success) {
	...
	const onFinishCreate = async (values: any) => {
		...
		if (createdMeter) {
			// Associate
			const success = await associateWaterMeterToClient(clientForMeters.rut, createdMeter.id);
			if (success) {
