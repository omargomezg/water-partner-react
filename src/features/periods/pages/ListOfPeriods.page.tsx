...
import { TableProps, Space, Button, Pagination, Table, Typography } from 'antd';
...
import { ButtonCreatePeriodComponent } from '../components/ButtonCreatePeriod.component';
...
const columns: TableProps<Period>['columns'] = [
	{
		...
		render: (_, record) => {
			return (
				<>
					<Paragraph style={{ marginBottom: '0px' }}>{record.name}</Paragraph>
	...
	},
	{
		title: 'Action',
		key: 'action',
		render: (_, record: Period) => <RowButtons period={record} />,
	},
];
...
export const ListOfPeriodsPage = () => {
	...
	const handleOnCreate = (period: Period) => {
		fetchPeriods();
	};
		...
		<>
			<div style={{ marginBottom: '16px', textAlign: 'right' }}>
				<Space>
					...
					<ButtonCreatePeriodComponent onCreate={handleOnCreate} />
				</Space>
			</div>
			<Table<Period>
				style={{ width: '100%' }}
...
const RowButtons: FC<RowButtonsProps> = ({ period }) => {
	...
	return (
		<Space>
			...
			<Button type="link" onClick={() => navigate(`/configurations/periods/${period.id}/edit`)}>
				<EditOutlined />
			</Button>
		</Space>
	);
