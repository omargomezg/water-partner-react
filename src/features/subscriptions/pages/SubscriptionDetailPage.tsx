...
export const SubscriptionDetailPage: FC = () => {
	...
	useEffect(() => {
		if (subscriptionId) {
			const fetch = async () => {
				const { data } = await apiClient<Subscription>(`/api/subscriptions/${subscriptionId}`);
	...
	return (
		<Spin spinning={!subscription}>
			...
			<Flex gap="middle" style={{ width: '100%' }} vertical={isMobile}>
				<CardStyle title="Propietario o Titular" style={{ flex: 2, height: '100%' }}>
					<Row gutter={16}>
						<Col md={8}>
							<ParagraphWithTitle title="Nombre" text={subscription?.owner?.fullName} />
						</Col>
						<Col md={8}>
							<ParagraphWithTitle title="Correo electrónico" text="jianasd@sds.com" />
						</Col>
						<Col md={8}>
				...
				<CardStyle
					title="Usuarios"
					extra={<AddUserComponent subscriptionId={subscription.id} refresh={setUsers} />}
					style={{ flex: 1, height: '100%' }}
				>
					<p>
						Cuentas adicionales vinculadas a este servicio habilitadas para consultar saldos, ver
						consumos y gestionar pagos.
					</p>
					{subscription.users &&
...
const ParagraphWithTitle: FC<ParagraphWithTitleProps> = ({ title, text }) => {
	return (
		<p>
			<span style={{ fontWeight: 'bold', color: '#413f3f', width: '180px' }}>{title}: </span>
			{text}
		</p>
	);
};
