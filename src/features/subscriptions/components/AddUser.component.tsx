...
export const AddUserComponent: FC<AddUserComponentProps> = ({ subscriptionId, refresh }) => {
	...
	return (
		<>
			...
			<Modal title="Agregar usuario" open={open} onCancel={handleCloseModal} onOk={form.submit}>
				<p>Solo necesitamos estos datos, eventualmente el usuario podría agregar otros.</p>
				<Form<Client> layout="vertical" form={form} onFinish={handleSave}>
					<Form.Item label="Rut" name="rut" rules={[{ required: true }]}>
						<RutInput />
					</Form.Item>
					<Form.Item label="Nombre" name="fullName" rules={[{ required: true }]}>
						<Input />
					</Form.Item>
					<Form.Item
