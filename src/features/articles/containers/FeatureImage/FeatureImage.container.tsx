...
export const FeatureImageContainer: React.FC = () => {
	...
	const fetchImage = async () => {
		const response = await apiClient.get(`/api/feature-image/${articleId}`).catch(() => null);
	...
	return (
		<>
			...
			>
				<Button type="link" onClick={() => navigate(`/articles/${articleId}/edit`)}>
					Cancelar
				</Button>
				{!imageId ? (
			...
			<Row gutter={16}>
				<Col span={12}>
					{file.img && <img src={file.img} alt={file.alt} style={{ width: '100%' }} />}
