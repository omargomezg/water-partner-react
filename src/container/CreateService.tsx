import { Button, Drawer, Flex, Form, Input, message, Select, SelectProps, Spin } from 'antd';
...
export const CreateService: FC<CreateServiceProps> = ({ onClose }) => {
	...
	useEffect(() => {
		const fetch = async () => {
			try {
				const { data } = await apiClient.get<MetaDataResponse>('/api/subscriptions/metadata');
	...
	const handleChangeOwner = async (rut: string) => {
		try {
			const { data } = await apiClient.get<PageResponse<Client>>(`/api/clients`, {
				params: {
					rut,
				},
			});
			...
			message.error('No se pudo cargar el cliente. Por favor, verifica el RUT ingresado.');
		}
	};
	...
	return (
		<>
			<Drawer open={open} onClose={handleClose} title="Crear servicio" footer={footer}>
				<Spin spinning={loading}>
					<Form id="create-service-form" layout="vertical" form={form} onFinish={handleFinish}>
