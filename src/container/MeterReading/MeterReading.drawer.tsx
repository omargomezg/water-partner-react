...
const MeterReadingDrawer = () => {
	...
	if (validHistory.length > 0) {
		...
		if (
			currentRecord?.reading &&
			currentRecord.reading > 0 &&
			validHistory[0].reading === currentRecord.reading
		) {
			...
			<Form layout="vertical" form={form} onFinish={onFinish}>
					...
					{previousReading ? (
						<Space>
							<Typography.Title level={4} style={{ margin: 0 }}>
								{previousReading.reading} m³
							</Typography.Title>
							<Typography.Text type="secondary">
								({dayjs(previousReading.readingDate).format('DD/MM/YYYY')})
							</Typography.Text>
						</Space>
					) : (
				...
				>
					<InputNumber
						style={{ width: '100%' }}
						min={0}
						placeholder="Ingrese valor en m3"
						size="large"
					/>
				</Form.Item>
			</Form>
